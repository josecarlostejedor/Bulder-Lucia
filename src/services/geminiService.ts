...
export async function analyzeWall(imageData: string, prompt: string, width: number, height: number): Promise<Itinerary> {
  ...
  if (!apiKey || !apiKey.startsWith("AIza")) {
    throw new Error(`Error: No hay clave de API válida. Detectado: "${apiKey.substring(0,4)}...". Revisa Vercel y haz Redeploy.`);
  }

  const ai = new GoogleGenAI({ apiKey });
  
  // 'gemini-flash-latest' es el alias para el modelo estable (1.5 Flash) que tiene 1500 peticiones/día.
  // Los modelos que empiezan por 'gemini-3' o 'gemini-2' suelen ser Preview/Experimental y tienen el límite de 20.
  const modelsToTry = ["gemini-flash-latest", "gemini-3-flash-preview"];
  let lastError = null;

  for (const modelName of modelsToTry) {
    try {
      console.log(`[${VERSION}] Intentando con: ${modelName}`);
      ...
      console.warn(`[${VERSION}] Fallo con ${modelName}:`, e.message);
      lastError = e;
      // Si es un error de cuota (429), esperamos un poco antes de probar el siguiente
  ...
  throw lastError || new Error("Fallo total de modelos.");
}
